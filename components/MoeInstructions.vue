<template>
	...
	>
		<template #item="item">
			<section class="flex flex-row w-1/2 h-1/4">
				<textarea
					...
					type="text"
				/>
				<p v-else class="bg-gray-600 rounded rounded-r-none p-1 w-48">
					{{ item.element }}
				</p>
				<div class="flex flex-col gap-1">
					<button v-if="editing === item.index" class="w-28 rounded-l-none rounded-br-none rounded bg-gray-400 px-2 h-1/2 py-1" @click="disabled=false; editing = -1">
						Fertig
					</button>
					<button v-else class="w-28 rounded-l-none rounded-br-none rounded bg-gray-400 px-2 h-1/2 py-1" @click="disabled=true; editing = item.index">
						Bearbeiten
					</button>
					<button class="w-28 rounded rounded-tr-none rounded-l-none bg-gray-400 px-2 h-1/2 py-1" @click="removeInstruction(item.index)">
						Entfernen
					</button>
				</div>
		...
		<template #footer>
			<button class="rounded bg-gray-400 px-2 py-1 w-1/5" @click="addInstruction">
				Schritt hinzufügen
			</button>
		</template>
	</draggable>
